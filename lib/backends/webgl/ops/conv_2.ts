...
export class WebGLConv extends Conv {
  initialize(attributes: Attribute) {
    ...
    this.activation = attributes.getString('__activation', '');
  }
  run(inferenceHandler: WebGLInferenceHandler, inputs: Tensor[]): Tensor[] {
    ...
    inferenceHandler.runProgram(this.artifacts[1], runDatas[1]);
  ...
  createDotProductProgramInfo(
    ...
    const sharedDimReadSize =  // blendEnabled && inferenceHandler.session.backend.matmulMaxBatchSize ?
                               // this.calcSharedDimReadSize(inferenceHandler.session.backend.matmulMaxBatchSize,
                               // sharedDim) :
        sharedDim;
    ...
    const shaderSource = `
    ${
        this.activation ? `float _a_a(float v) {
      return max( v, 0.0 );
    }` :
                          ''}
    float process(int indices[${rank}]) {
      ...
      return ${this.activation ? '_a_a(sum)' : 'sum'};
    }`;
    return {
